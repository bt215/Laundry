...
class JeniscuciController extends Controller
{
    public function index($id)
    {
        if(Auth::user()->level=="admin"){
            ...
            ->where('jenis_cuci.id_jenis_cuci',$id)
            ->get();
    ...
    public function update($id,Request $request){
        ...
        $ubah=Jeniscuci::where('id_jenis_cuci',$id)->update([
    ...
    public function destroy($id){
        if(Auth::user()->level=="admin"){
        $hapus=Jeniscuci::where('id_jenis_cuci',$id)->delete();
        $status=1;
    ...
    public function tampil(){
        if(Auth::user()->level=="admin"){
            ...
            foreach ($datas as $dt_jc){
                $jeniscuci[] = array(
                    'id_jenis_cuci' => $dt_jc->id_jenis_cuci,
